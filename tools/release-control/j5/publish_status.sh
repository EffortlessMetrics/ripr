...
status_repo="$RUNNER_TEMP/ripr-j5-status-repo"
status_branch=control/0.11.0-live-j5-status
mkdir -p "$packet"
if ! test -f "$packet/receipt.json" && ! test -f "$packet/failure.json"; then
  printf '%s\n' '{"schema":"ripr.source_promotion_j5_builder_failure.v1","status":"failed","phase":"workflow_or_shell_initialization","reason":"builder produced neither success nor failure receipt"}' \
    > "$packet/failure.json"
fi
...
git -C "$status_repo" commit -m "release(control): retain live J5 run $GITHUB_RUN_ID"
