...
def select_owner(base: Row | None, j2: Row | None, source: Row | None, actual: int) -> tuple[Row, str]:
    ...
    if not covering:
        ...
        raise SystemExit(
            f"reviewed J4 process count exceeds every declared maximum for "
            f"{candidates[0].path if candidates else '<unknown>'}|"
            f"{candidates[0].pattern if candidates else '<unknown>'}: "
...
def main() -> int:
    if len(sys.argv) != 7:
        raise SystemExit(
            "usage: reconcile_process_policy_v2.py <old-source-policy> <j2-policy> "
    ...
    for key in keys:
        ...
        row = Row(selected.path, selected.pattern, selected.maximum, selected.owner, selected.reason, selected.origin)
        result.append(row)
        decisions.append(
            {
                "path": row.path,
    ...
    receipt = {
        ...
        "invariants": [
            ...
            "both-changed rows use an existing declared maximum that covers the reviewed tree",
            "no maximum is widened implicitly; uncovered counts fail closed",
