...
fn print_help() {
    println!(
        "usage: cargo xtask first-pr [--root <path>] [--gap-ledger <path>] [--first-action <path>] [--review-comments <path>] [--agent-packet <path>] [--gate-decision <path>] [--receipts-dir <path>] [--out-dir <path>] [--check]"
    );
}
...
fn render_start_here_packet(root: &Path, options: &FirstPrOptions) -> Value {
    ...
    let artifacts = vec![
        ...
        artifact_status(
            ...
            Some(format!(
                "ripr review-comments --root {} --base <base> --head <head> --gap-ledger {} --out {}",
                options.root, options.gap_ledger, options.review_comments
            )),
        ),
        artifact_status(
            root,
