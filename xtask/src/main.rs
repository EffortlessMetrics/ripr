...
fn routed_rust_workflow_contract_violations_for_repo() -> Result<Vec<String>, String> {
    ...
    // Source-parent workflow topology predates the swarm-only routed runner
    // contract. Keep the generic workflow/security checks active, while only
    // applying routed-runner invariants when that topology is present.
    if !workflow.contains("scratch_status: ${{ steps.scratch.outputs.status }}") {
        return Ok(Vec::new());
    }
...
fn check_campaign() -> Result<(), String> {
    let mut violations = Vec::new();
    let campaign_doc = Path::new("docs/IMPLEMENTATION_CAMPAIGNS.md");
    if !campaign_doc.is_file() {
        violations.push("docs/IMPLEMENTATION_CAMPAIGNS.md is missing".to_string());
    }
    let workflow = Path::new(".github/workflows/source-of-truth.yml");
    if workflow.is_file() {
        let text = fs::read_to_string(workflow)
            .map_err(|err| format!("failed to read {}: {err}", workflow.display()))?;
        if !text.contains("run: cargo xtask check-campaign") {
            violations.push("source-of-truth workflow must invoke check-campaign".to_string());
        }
    }
    finish_policy_report(
        PolicyReportSpec {
            ...
            why_it_matters: "The source-of-truth workflow must expose a stable campaign consistency command.",
            fix_kind: FixKind::AuthorDecisionRequired,
            recommended_fixes: &[
                "Keep the implementation campaign document and source-of-truth workflow present.",
            ],
