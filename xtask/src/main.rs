...
pub(crate) use reports::release_server::{
    ...
    validate_release_server_staging_inventory, write_release_server_file_atomic,
};
#[cfg(test)]
...
/// Crate-root exports declared directly in `lib.rs`.
///
/// This is the narrow, source-authoritative view of the public surface: the
/// `pub mod` and `pub use` lines of the crate root, recorded in
/// `policy/public_api.txt`. The source-trusted resolved-tree validator runs the
/// source-parent build of this gate against that file, so its format is a
/// cross-repository contract and must not be widened in place.
...
fn check_public_api() -> Result<(), String> {
    ...
    // Crate-root declarations against the source-authoritative ledger.
    let root_allowed = read_public_api_allowlist("policy/public_api.txt")?;
    let root_actual = public_api_exports(Path::new("crates/ripr/src/lib.rs"))?;
    let root_allowed_set = root_allowed.iter().cloned().collect::<BTreeSet<_>>();
    let root_actual_set = root_actual.iter().cloned().collect::<BTreeSet<_>>();
    for line in &root_actual {
        if !root_allowed_set.contains(line) {
            violations.push(format!(
  ...
  update policy/public_api.txt only when this is an intentional public contract"
    ...
    for line in &root_allowed {
        if !root_actual_set.contains(line) {
            violations.push(format!(
    ...
    // Transitive module-level surface against the recorded surface ledger.
    let allowed = read_public_api_allowlist("policy/public_api_surface.txt")?;
    ...
    for line in &actual {
        if !allowed_set.contains(line) {
            violations.push(format!(
                "public API export is not allowlisted: {line}
  update policy/public_api_surface.txt only when this is an intentional public contract"
            ));
        }
    }
    for line in &allowed {
    ...
    finish_policy_report(
        PolicyReportSpec {
            ...
            recommended_fixes: &[
                ...
                "If the public export is intentional, update policy/public_api.txt (crate-root declarations) or policy/public_api_surface.txt (transitive surface) and explain the contract in the PR.",
