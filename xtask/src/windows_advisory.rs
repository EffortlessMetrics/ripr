...
mod tests {
    ...
    use super::*;
    fn packaged_workflow() -> Option<String> {
        std::fs::read_to_string(".github/workflows/windows-packaged-qualification.yml").ok()
    }
    ...
    fn packaged_qualification_workflow_is_immutable_and_non_publishing() {
        let Some(workflow) = packaged_workflow() else {
            return;
        };
        let lines = workflow
            .lines()
