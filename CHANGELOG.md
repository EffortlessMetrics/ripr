...
- Added `ripr first-action`, a read-only advisory report producer that writes
  `target/ripr/reports/first-useful-action.{json,md}` from explicit existing
  PR guidance, assistant proof, PR ledger, baseline delta, receipt, gate,
  coverage/frontier, and editor-context artifact paths without hidden analysis
  reruns, source edits, generated tests, provider calls, mutation execution, or
  default CI blocking.
