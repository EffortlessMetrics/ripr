...
mod tests {
    ...
    fn deadline_kills_pipe_inheriting_descendants_without_blocking_the_reader() -> Result<(), String>
    {
        ...
        let started = Instant::now();
        // The setup must complete before the deadline fires, or the tree kill
        // lands between `Start-Process` and `Set-Content` and the PID marker is
        // never written -- the test then fails without ever evaluating its
        // discriminator. Cold-starting two PowerShell processes on a loaded
        // hosted Windows runner does not reliably fit in five seconds. Fifteen
        // clears that setup while staying well inside the thirty-second bound
        // asserted below, and the descendant still sleeps for sixty, so the
        // "was the inherited writer actually terminated" check is unchanged.
        let result = collect_output_with_deadline(
            &mut command,
