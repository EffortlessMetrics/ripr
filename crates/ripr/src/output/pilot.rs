...
mod tests {
    ...
    fn pilot_summary_renderers_omit_recommendation_when_no_actionable_seams() {
        // No actionable seams: only governed classes survive ranking,
        // and they are excluded by `class_rank`. All three renderers
        // must produce the "nothing actionable" branches without
        // crashing or emitting recommendation copy.
        let entries = [
            classified_with(
                SeamGripClass::StronglyGripped,
                "src/a.rs",
    ...
    fn why_line_uses_static_discriminator_summary_when_no_missing_discriminator() {
        // No `missing_discriminators` set, but the discriminate stage
        // carries a non-empty summary. `why_line` must surface that
        // summary in the "static discriminator summary: ..." form.
    ...
    fn why_line_falls_back_to_class_label_when_no_summary_or_missing_discriminator() {
        // Neither a `missing_discriminators` entry nor a non-empty
        // discriminate summary: `why_line` falls back to "{class}
        ...
        assert_eq!(why_line(&entry), "ungripped static seam evidence");
    }
}
