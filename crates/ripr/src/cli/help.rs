...
Quick start:
  ...
  ripr agent start --root . --seam-id f3c9e4d21a0b7c88 --out target/ripr/workflow
...
Subcommands:
  start      Write a source-edit-free agent workflow manifest.
...
Run `ripr agent start --help`, `ripr agent brief --help`,
`ripr agent packet --help`, or `ripr agent verify --help` for JSON-only agent
surfaces. Run `ripr agent receipt --help` for the verification receipt surface,
and `ripr agent status --help` for the artifact status lens.
"#;
...
Options:
  ...
  --seam-id ID     Seam ID to carry through the workflow checklist.
  --out PATH       Output directory for agent-workflow.json and agent-workflow.md.

The start command writes a source-edit-free workflow manifest for LLM agents.
It records the before snapshot, packet, brief, after snapshot, verify, receipt,
status, and future review-summary artifact paths plus the commands to produce
them. It remains advisory and static; it does not validate the seam by running
analysis, run mutation testing, generate tests, edit files, change cache
...
mod tests {
    ...
    fn command_specific_help_usage_lines_are_stable() {
        ...
        assert!(AGENT_START_HELP.contains("agent-workflow.json"));
