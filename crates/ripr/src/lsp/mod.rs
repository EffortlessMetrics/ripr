...
pub fn serve() -> Result<(), String> {
    ...
    while let Some(message) = protocol::read_lsp_message(&mut reader)? {
        if let Some(response) = responses::response_for_message(&message) {
            protocol::write_lsp_message(&mut writer, &response)?;
            continue;
        }

        if responses::should_publish_diagnostics(&message) {
            publish_workspace_diagnostics(&mut writer)?;
            continue;
        }

        if responses::is_exit_request(&message) {
            break;
        }

        if let Some(id) = protocol::extract_id(&message) {
            protocol::write_lsp_message(&mut writer, &responses::null_result(&id))?;
        }
    }
...
fn publish_workspace_diagnostics(writer: &mut impl Write) -> Result<(), String> {
    ...
    let notifications = responses::diagnostic_notifications(&output.findings);
    for notification in notifications {
        protocol::write_lsp_message(writer, &notification)?;
    }
    Ok(())
}
