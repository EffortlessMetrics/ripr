...
mod tests {
  ...
  "findings": []
    ...
    fn agent_review_summary_markdown_next_command_offers_powershell_variant() -> Result<(), String>
    {
        ...
        let powershell_form = "```powershell\n$staging = 'target/ripr/workflow/before.repo-exposure.json.ripr-staging'; Remove-Item -LiteralPath $staging -Force -ErrorAction SilentlyContinue; $process = Start-Process -FilePath 'ripr' -ArgumentList @('check', '--root', '.', '--mode', 'draft', '--format', 'repo-exposure-json') -RedirectStandardOutput $staging -NoNewWindow -Wait -PassThru; if ($process.ExitCode -ne 0) { Remove-Item -LiteralPath $staging -Force -ErrorAction SilentlyContinue; throw \"ripr exited with code $($process.ExitCode)\" }; Move-Item -LiteralPath $staging -Destination 'target/ripr/workflow/before.repo-exposure.json' -Force\n```\n";
        assert!(
