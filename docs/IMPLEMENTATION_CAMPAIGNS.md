...
| `report/recommendation-precision` | done | Added `cargo xtask recommendation-calibration`, which joins PR guidance, calibration corpus expectations, optional outcome receipts, suppression state, target placement, static movement, and latency into advisory JSON and Markdown reports without changing CI blocking defaults. |
