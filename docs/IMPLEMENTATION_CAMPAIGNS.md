...
| `ci/language-aware-grouping` | ready | Update generated GitHub CI summaries to group advisory output by language only when `[languages]` declares more than Rust, keeping Rust-default behavior identical and gate authority unchanged. Editor projection now consumes the same preview labeling and static-limit boundary, so the CI grouping slice can start. |
...
- Continue with `ci/language-aware-grouping`. Keep Rust-default generated CI
  output and gate authority unchanged, and group advisory preview output only
  when `[languages]` declares more than Rust. Do not fold policy, gate,
  mutation, provider, generated-test, source-edit, inline-comment,
  branch-protection, or default-CI changes into the CI grouping slice.
