...
| `calibration/cargo-mutants-v1` | done | Adds advisory `cargo xtask mutation-calibration` report generation. Imported cargo-mutants JSON/output is joined to static `SeamGripClass` evidence by `seam_id` first and normalized file/line second; unmatched runtime mutants remain visible; runtime mutation vocabulary stays inside `target/ripr/reports/mutation-calibration.{json,md}`. |
