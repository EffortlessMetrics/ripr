...
| `lsp/agent-loop-copy-commands` | done | Expose command-oriented copy actions for agent packet, brief, after-snapshot, verify, and receipt commands from seam diagnostics. No automatic edits, CodeLens, inlay hints, semantic tokens, or unsaved-buffer overlays. |
| `operator/verify-receipt-status` | ready | Report missing before/after snapshots, agent verify JSON, agent receipt JSON, movement counts, and next commands that match editor copy commands. |
