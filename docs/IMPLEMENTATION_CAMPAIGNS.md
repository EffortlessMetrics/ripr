...
| `agent/centralize-loop-command-templates` | done | Added a shared Rust command-template module for the workflow and editor-agent path profiles, then routed agent status, agent brief next commands, pilot follow-up commands, LSP copy actions, generated CI path references, and operator cockpit commands through it without changing pinned output. |
| `agent/workflow-manifest` | ready | Add `ripr agent start --root . --seam-id <id> --out target/ripr/workflow` after command templates are centralized, writing a source-edit-free workflow manifest with artifact paths and commands. |
