...
| `badge/ripr-count-v1` | ready | `ripr check --format badge-json` and `--format badge-shields` wire the renderer to the CLI surface. The next PR also removes the `#![allow(dead_code)]` gate in `output/badge.rs` and the matching entry in `.ripr/allow-attributes.txt`. |
| `test-intent/v1` | ready | `.ripr/test_intent.toml` loader marks declared tests intentional in test-efficiency output; `ripr+` excludes them. Blocker cleared by `test-efficiency/report-and-metrics`. |
