...
| `analysis/python-preview-adapter` | blocked | Add the Python syntax-first adapter, the Python fixture corpus, and the preview labeling without changing Rust behavior, default CI, or inline-comment defaults. Mirrors the TypeScript scaffold pattern: add the `rustpython-parser` Cargo dependency approved by the corrected ADR 0009, add `PythonAdapter` wiring, route `.py` through the language router, and parse changed `.py` files when `[languages]` includes `python`. Empty findings until later sub-slices extract owners, tests, and assertions. |
