...
| `badge/publish-main-endpoint` | done | `.github/workflows/publish-badge-endpoint.yml` publishes repo-scoped Shields JSON to GitHub Pages from `main` runs only (`if: github.event_name == 'push' && github.ref == 'refs/heads/main'`). Endpoints: `https://effortlessmetrics.github.io/ripr/badges/ripr.json` and `https://effortlessmetrics.github.io/ripr/badges/ripr-plus.json`. Root `README.md` renders them via `img.shields.io/endpoint`. First-party `actions/configure-pages`, `actions/upload-pages-artifact`, `actions/deploy-pages` introduce no new `curl`/`wget` patterns, so `policy/network_allowlist.txt` needed no entry; `policy/workflow_allowlist.txt` registers the new workflow line budget. PR workflows do not publish. |
