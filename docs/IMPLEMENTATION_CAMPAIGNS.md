...
- Campaign 22, First Useful Action, is now the active campaign. Its report
  ...
  pinned, the editor status projection is in place, and the next ready item is
...
| `lsp/first-useful-action-status` | done | VS Code status and Show Status now read an existing workspace-matched `first-useful-action.json` and project the top action, seam, target, and verify/receipt availability without adding diagnostics or editor decorations; stale editor state remains visible before the report is named. |
...
- Continue with `docs/first-useful-action-workflow`. Keep dogfood receipts and
  closeout blocked until the workflow docs explain how developers, reviewers,
  and coding agents use the first-action report.
