...
| `docs/gate-baseline-workflow` | done | Added baseline creation and refresh guidance for reviewed historical debt, including expected `baseline-check` behavior and the boundary between baselines, waivers, and suppressions. |
...
- `ci/gate-decision-summary-polish` is the next ready item. Keep the generated
  summary readable from the first screen while preserving advisory defaults.
