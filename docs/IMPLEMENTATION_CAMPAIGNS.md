...
| `adr/python-parser-substrate` | done | Pinned the Python parser-substrate decision before any Cargo dependency or adapter code lands, mirroring how ADR 0008 (`oxc_parser`) was sequenced ahead of the TypeScript scaffold. Added `docs/adr/0009-python-parser-substrate.md`, registered it in `docs/adr/README.md`, and added the approved-decision comment for `ruff_python_parser` to `policy/dependency_allowlist.txt`. No Cargo dependency, no adapter code, no behavior change. Closed #770. |
