...
| `ci/badge-artifacts` | ready | CI uploads `target/ripr/reports/ripr-badge*.json` and `ripr-plus-badge*.json`, links them from the report index, and shows counts in the GitHub step summary. Advisory by default. Both blockers (`badge/ripr-plus-count-v1` and `suppressions/v1`) are now done. |
