...
| `badge/ripr-plus-count-v1` | blocked | `ripr check --format badge-plus-json` adds actionable test-efficiency findings (excluding intent) to the `ripr+` count. Blocked on `test-intent/v1` (the `ripr` badge CLI shipped in `badge/ripr-count-v1`). |
