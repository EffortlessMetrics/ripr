...
| `analysis/language-adapter-boundary` | active | Introduce `LanguageId`, `LanguageAdapter`, and the language router inside `crates/ripr/src/analysis/` without changing Rust fixture, golden, or output schema behavior. The `RustAdapter` reference type is wired into workspace discovery as a functional no-op so the seam is alive; `LanguageFacts`, `OwnerFact`, `TestFact`, `AssertionFact`, `OwnerKind`, and `StaticLimitKind` shells land alongside their production consumers in the rust-adapter-behind-boundary work item. |
