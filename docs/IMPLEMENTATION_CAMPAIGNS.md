...
| `adr/python-parser-substrate` | active | Pin the Python parser-substrate decision before any Cargo dependency or adapter code lands, mirroring how ADR 0008 (`oxc_parser`) was sequenced ahead of the TypeScript scaffold. Adds `docs/adr/0009-python-parser-substrate.md`, registers it in `docs/adr/README.md`, and adds the approved-decision comment for `ruff_python_parser` to `policy/dependency_allowlist.txt`. No Cargo dependency, no adapter code, no behavior change. Tracked as #770. |
| `analysis/python-preview-adapter` | blocked | Add the Python syntax-first adapter, the Python fixture corpus, and the preview labeling without changing Rust behavior, default CI, or inline-comment defaults. Mirrors the TypeScript scaffold pattern: add the `ruff_python_parser` Cargo dependency approved by ADR 0009, add `PythonAdapter` wiring, route `.py` through the language router, and parse changed `.py` files when `[languages]` includes `python`. Empty findings until later sub-slices extract owners, tests, and assertions. |
