...
| `badge/repo-scope-artifacts` | ready | New `cargo xtask repo-badge-artifacts` task that analyzes the full repo baseline (not a diff) and writes `repo-ripr-badge.json`, `repo-ripr-badge-shields.json`, `repo-ripr-plus-badge.json`, `repo-ripr-plus-badge-shields.json`, and `repo-ripr-badges.md`. Native JSON gains a `scope` field on a bumped `schema_version`; Shields projection stays exactly four fields. Existing diff-scoped `cargo xtask badge-artifacts` is unchanged. Discovered during dogfood preflight: a no-diff `main` run of the diff-scoped task produces `ripr 0 brightgreen` mechanically — that is "nothing changed," not "the repo is clean," and is unsafe as a public README signal. |
| `badge/publish-main-endpoint` | blocked | Trunk-only Shields endpoint published from `main`; README badges point at it. Requires `policy/network_allowlist.txt` entry **and** repo-scoped artifacts. Blocked on `badge/repo-scope-artifacts`. |
