...
| `badge/ripr-plus-count-v1` | ready | `ripr check --format badge-plus-json` adds actionable test-efficiency findings (excluding declared intent) to the `ripr+` count. Both blockers (`badge/ripr-count-v1` and `test-intent/v1`) are now done. |
