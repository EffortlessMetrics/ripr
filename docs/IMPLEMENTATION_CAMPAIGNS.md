...
| `ci/editor-agent-artifacts` | ready | Make the generated GitHub workflow upload non-blocking editor-agent loop artifacts: pilot summary, repo exposure, agent brief, agent packet, agent verify, agent receipt, operator cockpit, SARIF when enabled, and badge JSON. |
| `docs/full-evidence-loop` | blocked | Blocked on `ci/editor-agent-artifacts`. Update quickstart and installed-user docs around the real diagnostic-to-receipt loop and state that `ripr init` is optional policy materialization, not activation. |
