...
| `analysis/rust-adapter-behind-boundary` | active | Move Rust fact extraction behind the `LanguageAdapter` trait as the reference adapter while preserving every existing Rust fixture, golden, capability, and output contract. Pipeline orchestration loads diffs, dispatches `analyze_diff`/`analyze_repo` to the adapter, and applies the language-neutral sort + summary on returned `Finding`s. |
