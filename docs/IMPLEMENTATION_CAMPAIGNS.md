...
| `badge/summary-renderer-v1` | ready | Private `BadgeSummary` types in `output::badge` render `badge-json` and `badge-shields` from `CheckOutput`. Public API stays unchanged. Blockers cleared by `test-efficiency/report-and-metrics`. |
