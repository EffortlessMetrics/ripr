...
| `output/language-metadata` | active | Add additive optional `language` field on each `Finding` (Rust adapter sets `"rust"`, omitted otherwise) and `language_status` (preview adapters set `"preview"`; omitted for Rust per spec). `LanguageId`/`LanguageStatus` move to `domain::language` as pure-data enums so output renderers can serialize without depending on the analysis layer. JSON renderer emits the fields when populated; goldens refreshed accordingly. `owner_kind` and `static_limit_kind` remain deferred until preview adapters populate them. |
