...
```bash
cargo test -p ripr review_feedback
cargo xtask check-campaign
cargo xtask goals next
cargo xtask check-doc-index
cargo xtask check-traceability
...
| `review-feedback/outcome-receipts` | done | Added `ripr review-feedback` and Review Feedback Outcome Receipt schema 0.1 for useful, noisy, wrong-line, already-covered, wrong-target, summary-only-correct, suppressed-correctly, and missing-recommendation local feedback without telemetry or external services. |
| `report/recommendation-precision` | ready | Emit a recommendation precision report that joins PR guidance, calibration corpus expectations, optional outcome receipts, suppression state, target placement, and latency without changing CI blocking defaults. |
