...
| `lsp/seam-evidence-hover-v1` | ready | Hover renders the RIPR evidence path with cited related tests. **PR #211 is merged** (pre-4B evidence-rich hover over the current Finding / AnalysisSnapshot model); this seam-native hover will extend or revise it once the seam model and diagnostics schema are settled. |
...
| `docs/agent-dispatch-workflow-v1` | ready | How a coding agent uses a seam packet to write the missing test. |
