...
| `agent/workflow-manifest` | ready | Add `ripr agent start --root . --seam-id <id> --out target/ripr/workflow` now that command templates are centralized, writing a source-edit-free workflow manifest with artifact paths and commands. |
