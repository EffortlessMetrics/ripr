...
| --- | --- | --- |
| `dynamic_dispatch` | The call target or behavior may be selected dynamically, such as dynamic calls through `obj[name]` or `getattr(obj, name)`. | Treat the finding as advisory. Prefer a focused test that observes the concrete runtime target or result. |
