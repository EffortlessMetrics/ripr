...
| Policy Operations and Promotion Readiness (Lane 2 tracker) | Make policy adoption operational with current safe ceiling, next safe action, blockers to stricter modes, policy history, and read-only promotion packets. | Open: [Policy operations](policy/POLICY_OPERATIONS.md) and `.ripr/goals/lane2-policy-operations.toml` define the focused tracker and PR order. RIPR-SPEC-0039 defines the policy operations report contract, `ripr policy operations` now writes the first operator packet, RIPR-SPEC-0041 defines the policy history trend contract, `ripr policy history` now writes the advisory trend packet, RIPR-SPEC-0042 defines manual-review promotion packets, `ripr policy promote --to ...` now writes those packets, RIPR-SPEC-0044 defines default-blocked preview evidence promotion packets, `ripr policy preview-promote` now writes preview evidence promotion packets, and [Policy operations workflow](POLICY_OPERATIONS_WORKFLOW.md) documents maintainer use; generated-CI and closeout remain planned follow-up slices. |
