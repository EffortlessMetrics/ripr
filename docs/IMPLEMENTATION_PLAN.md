...
| First Useful Action (22) | Compress existing editor, PR, ledger, proof, receipt, optional gate, coverage/grip, and staleness evidence into one advisory next test action. | Active: RIPR-SPEC-0020 defines the report contract, the routing corpus is pinned, `ripr first-action` writes the read-only advisory report, and generated CI now surfaces it as advisory summary/artifact content; editor status projection is ready, with workflow docs, dogfood receipts, and closeout still blocked behind that projection. |
...
pins expected statuses and fallback outputs, and `ripr first-action` now writes
the read-only report from explicit artifacts. CI summary projection and editor
status projection are the next ready work items.
