...
| PR Review Guidance (13) | Project existing RIPR evidence into bounded pull-request review guidance without making CI blocking or turning RIPR into a free-form reviewer. | Active: `review/pr-guidance-renderer` is done and `ci/run-pr-guidance-report` is ready. |
...
Markdown report, and the next PR should run that producer inside generated CI
before the existing summary and annotation consumer steps.
