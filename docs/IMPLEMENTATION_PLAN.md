...
| PR Inline Comment Publisher (26) | Make optional durable PR comments safe and explicit by planning, capping, deduplicating, and publishing only changed-line `review-comments` entries when configured. | Active: spec, fixture corpus, read-only publish plan, generated-CI opt-in wiring, workflow docs, and dogfood receipts are in place; `campaign/pr-inline-comment-publisher-closeout` is the next ready work item. |
...
checks repo-local publish-plan receipts without posting real PR comments. The
next ready slice is `campaign/pr-inline-comment-publisher-closeout`.
