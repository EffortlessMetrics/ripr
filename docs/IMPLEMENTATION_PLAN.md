...
| First-Hour UX (12) | Make new LSP-first and CI-first users successful without learning RIPR's internal report topology. | Active: `ci/pr-summary-surface` is the next ready item after the editor status and action-discoverability slices landed. |
...
annotation contract is pinned, the extension has a first-run status path, and
diagnostic actions are titled around user intent. The next slice keeps the CLI
as the shared engine while making generated CI summaries useful before artifact
download.
