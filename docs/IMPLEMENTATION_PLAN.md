...
| First-Hour UX (12) | Make new LSP-first and CI-first users successful without learning RIPR's internal report topology. | Active: `vscode/action-discoverability` is the next ready item after the editor first-run status path landed. |
...
the next slice keeps the CLI as the shared engine while making diagnostic
actions easier to discover from user intent.
