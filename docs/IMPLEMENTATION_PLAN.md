...
| Assistant Loop Health (23) | Summarize proof completeness, missing inputs, static evidence movement, recurring warnings, and next repair queues across one or more assistant proof reports. | Active: `spec/assistant-loop-health-report` is the first ready work item after First Useful Action settled the first-screen routing contract. |
...
design brief and starts with `spec/assistant-loop-health-report`. The campaign
measures whether assistant proof packets are complete, stuck, missing receipts,
or moving static evidence over time, without changing analyzer behavior,
ranking, gate semantics, LSP/editor behavior, mutation execution, provider
calls, source files, generated tests, or default CI blocking.
