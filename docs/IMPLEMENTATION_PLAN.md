...
| Policy Operations and Promotion Readiness (Lane 2 tracker) | Make policy adoption operational with current safe ceiling, next safe action, blockers to stricter modes, policy history, and read-only promotion packets. | Open: [Policy operations](policy/POLICY_OPERATIONS.md) and `.ripr/goals/lane2-policy-operations.toml` define the focused tracker and PR order. Spec, implementation, traceability, capability, output-schema, generated-CI, workflow, and closeout surfaces remain planned follow-up slices. |
