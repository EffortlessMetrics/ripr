...
RIPR-SPEC-0013 now pins the recommendation calibration report contract, the
PR-shaped calibration corpus is fixture-pinned, and `ripr review-feedback`
writes repo-local outcome receipts. The next ready item is
`report/recommendation-precision`. Campaign 15 is queued as the Calibrated Gate Policy lane so optional
