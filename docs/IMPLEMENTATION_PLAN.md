...
| Test-Oracle Assistant Proof (20) | Prove the full PR-time loop from changed Rust behavior to static evidence, PR/editor guidance, focused-test handoff, verification, receipt, and advisory CI/ledger projection. | Active: `spec/test-oracle-assistant-loop`, `fixtures/canonical-review-loop`, and `dogfood/test-oracle-assistant-receipt` are done; `docs/test-oracle-assistant-workflow` is ready; closeout remains blocked behind the workflow docs. |
