...
| First-Hour UX (12) | Make new LSP-first and CI-first users successful without learning RIPR's internal report topology. | Active: `vscode/first-run-status` is the next ready item after the PR guidance annotation contract was pinned. |
...
annotation contract is pinned, and the next slice keeps the CLI as the shared
engine while making the extension status path obvious from its own first screen.
