...
| Calibrated Gate Policy (15) | Define optional calibrated gates over existing PR-time evidence without changing advisory defaults or blurring static/runtime evidence. | Active: `docs/calibrated-gate-policy` is ready after spec, evaluator, fixture matrix, and explicit generated CI wiring. |
...
pins the decision matrix; and generated GitHub workflows now run gate
evaluation only when `RIPR_GATE_MODE` is explicitly configured, so
`docs/calibrated-gate-policy` is the next ready item.
