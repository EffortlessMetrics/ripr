...
| Calibrated Gate Policy (14) | Define optional calibrated gates over existing PR-time evidence without changing advisory defaults or blurring static/runtime evidence. | Active: RIPR-SPEC-0013 is pinned and `gate/policy-evaluator` is the next ready item. |
| Recommendation Quality Calibration (15) | Measure whether top CI, LSP, and PR recommendations are clear, correctly placed, low-noise, and correlated with better static evidence after one focused test. | Queued after Campaign 14 so recommendation quality can be measured separately from optional gate policy. |
...
workflow. Campaign 14 is active as the Calibrated Gate Policy lane:
RIPR-SPEC-0013 now pins optional gates as explicit policy over existing
evidence, remaining advisory by default, preserving acknowledgement paths, and
using runtime mutation calibration only as imported confidence evidence. The
next ready item is the read-only gate evaluator. Campaign 15 is queued as the
Recommendation Quality Calibration lane: it should measure whether the
completed PR, CI, and LSP recommendation surfaces are actionable, correctly
placed, suppressed or capped for the right reasons, and correlated with
unchanged, improved, regressed, resolved, or missing after-test evidence
without telemetry, generated tests, runtime mutation execution, or default CI
blocking.
