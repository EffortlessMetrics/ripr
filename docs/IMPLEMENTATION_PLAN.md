...
read-only report from explicit artifacts, generated CI and VS Code project it
as advisory next-action evidence, and
[First useful action workflow](FIRST_USEFUL_ACTION_WORKFLOW.md) explains how to
act on the report, verify static movement, and return receipts. Dogfood
receipts are the next ready work item.
