...
| Editor Agent Integration (10) | Make the saved-workspace editor loop and the agent CLI loop line up from diagnostic to evidence, packet/brief, focused test, verify, receipt, cockpit, CI, and install proof. | Active: `release/editor-agent-readiness-proof` and `campaign/editor-agent-integration-closeout`. |
...
`release/editor-agent-readiness-proof`. The 0.4 release-surface concerns from
#463 are carried as that release-readiness proof before closeout rather than as
