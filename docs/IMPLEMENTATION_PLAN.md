...
| Policy Readiness and Preview Evidence Governance (Lane 2 tracker) | Make policy decisions auditable across stable Rust evidence and preview-language evidence without changing advisory defaults. | Tracker: [#755](https://github.com/EffortlessMetrics/ripr/issues/755) and [Policy readiness](policy/POLICY_READINESS.md) define the focused Lane 2 board; readiness reporting, preview evidence policy, waiver aging, suppression health, baseline refresh guardrails, and exception ledger convergence are done; remaining work covers blocking guidance, advisory CI projection, and closeout. |
