...
def replace_once(text: str, old: str, new: str, label: str) -> str:
    ...
    if count != 1:
        raise SystemExit(f"{label} anchor count was {count}, expected exactly one")
...
loader_anchor = r'''    let swarm_ingest = read_text_lossy(Path::new("crates/ripr/src/output/swarm_ingest.rs"))?;
    let mut json_output = String::new();
'''
loader_replacement = r'''    let swarm_ingest = read_text_lossy(Path::new("crates/ripr/src/output/swarm_ingest.rs"))?;
...
main_text = replace_once(
    ...
    loader_anchor,
    loader_replacement,
    "source-promotion output-contract loader",
)

arm_anchor = r'''            "ingest_reason" => {
...
'''
arm_replacement = r'''            "ingest_reason" => {
                require_contract_value(
                    "crates/ripr/src/output/swarm_ingest.rs",
                    &swarm_ingest,
                    value,
...
main_text = replace_once(
    ...
    arm_anchor,
    arm_replacement,
    "source-promotion output-contract match arm",
)
main.write_text(main_text, encoding="utf-8")

docs = Path("docs/OUTPUT_SCHEMA.md")
docs_text = docs.read_text(encoding="utf-8")
marker = "## Source-promotion resolved-tree validation receipt"
if marker not in docs_text:
    docs_text = docs_text.rstrip() + r'''
...
test = Path("xtask/tests/output_contract_registry.rs")
if not test.exists():
    test.write_text(
        r'''use std::fs;
...
for transient in [
    Path(".github/workflows/repair-1605-output-contracts.yml"),
    Path(".github/scripts/repair_1605_output_contracts.py"),
]:
