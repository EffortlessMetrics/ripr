...
| Assistant loop health | `ripr assistant-loop health` writes advisory `assistant-loop-health.{json,md}` from explicit `test-oracle-assistant-proof` inputs, summarizes complete, partial, missing-input, static-movement, warning, and repair-queue states, and generated CI uploads the report only when proof artifacts exist. | `campaign/assistant-loop-health-closeout` |
