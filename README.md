...
<p align="center">
  ...
  <a href="https://marketplace.visualstudio.com/items?itemName=EffortlessMetrics.ripr"><img src="https://img.shields.io/badge/VS%20Marketplace-4%20installs-0078D4" alt="VS Marketplace Installs (manual)" /></a>
...
<!-- Public Shields endpoint JSON under badges/ is generated by `cargo xtask badges`; verify with `cargo xtask badges --check`. The public `ripr+` badge counts unresolved repo-scoped seam-native exposure gaps plus actionable test-efficiency findings under the configured policy — inbox-zero, not coverage. Diff-scoped artifacts remain PR summary and CI artifact evidence. See [docs/BADGE_POLICY.md](docs/BADGE_POLICY.md) and `deferred/hosted-badge-service` in [docs/DEFERRED.md](docs/DEFERRED.md). -->
