...
[![RIPR gaps](https://img.shields.io/endpoint?url=https://raw.githubusercontent.com/EffortlessMetrics/ripr/main/badges/ripr.json)](docs/BADGE_POLICY.md)
[![RIPR+ gaps](https://img.shields.io/endpoint?url=https://raw.githubusercontent.com/EffortlessMetrics/ripr/main/badges/ripr-plus.json)](docs/BADGE_POLICY.md)
...
For local LLM handoff state, `ripr agent status --root . --json` reads the
existing `target/ripr/workflow` and `target/ripr/reports` artifacts and reports
which loop step is missing next. It does not rerun analysis or edit source.
...
| Agent context | Compact context packet plus per-seam `write_targeted_test` and `inspect_static_limitation` packets carrying recommended test placement, nearest tests to imitate, candidate values, missing discriminators, patterns to imitate/avoid, and assertion templates. `ripr agent status --root . --json` now reports local LLM loop artifact state and the next missing command without rerunning analysis. | `agent/centralize-loop-command-templates` |
